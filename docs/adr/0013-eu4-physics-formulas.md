...
### Отрицательные
- `getRenderMode` пока не интегрирован в MapOverlay 2-pass (теоретическая функция)
...
1. **Интегрировать `getRenderMode` в MapOverlay 2-pass** (PASS 1): заменить `classifyLabelMode`
   на `getRenderMode({cameraDistance, screenSpineLength, screenArea, spineEligible})` +
   `spineEligible = isSpineEligible(...) && !hasSharpSpineTurn(...)`.
